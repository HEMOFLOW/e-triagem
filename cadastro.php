...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nome) || empty($data_nascimento) || empty($cpf) || empty($telefone) || empty($senha)) {
        ...
        try {
            ...
            $cpf_formatado_para_busca = substr($cpf, 0, 3) . '.' . substr($cpf, 3, 3) . '.' . substr($cpf, 6, 3) . '-' . substr($cpf, 9, 2);
            $stmt = $pdo->prepare("SELECT id FROM usuarios WHERE cpf = ? OR cpf = ?");
            $stmt->execute([$cpf, $cpf_formatado_para_busca]);
            if ($stmt->fetch()) {
                ...
                if ($stmt->execute([$nome, $data_nascimento, $cpf, $telefone, $email, $senha_hash])) {
