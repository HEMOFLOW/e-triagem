 ...
 * E-Triagem - Sistema de Doação de Sangue
 */
...
class Database {
    ...
    private $password = 'Academico@2025';
...
function initDatabase() {
    try {
        ...
        $stmt = $pdo->query("SELECT COUNT(*) FROM questionario_config");
        if ($stmt->fetchColumn() == 0) {
            $perguntas_padrao = [
                ['Você está em boas condições de saúde?', 1],
                ['Dormiu pelo menos 6 horas nas últimas 24h?', 1],
                ['Está alimentado?', 1],
                ['Está gripado, resfriado ou com febre?', 0],
                ['Fez cirurgia nos últimos 12 meses?', 0],
                ['Fez tatuagem ou piercing nos últimos 12 meses?', 0],
                ['Teve contato com pessoa com hepatite?', 0],
                ['Usou drogas ilícitas?', 0],
                ['Está gestante ou amamentando?', 0],
                ['Teve comportamento de risco para doenças sexualmente transmissíveis?', 0],
            ];
            $stmt = $pdo->prepare("INSERT INTO questionario_config (pergunta, resposta_correta) VALUES (?, ?)");
            foreach ($perguntas_padrao as $p) {
                $stmt->execute($p);
            }
        }
